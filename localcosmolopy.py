...
This is a local version of the distance.py file of the CosmoloPy package 
by Roban Hultman Kramer. It was updated to Python 3.7
http://pydoc.net/CosmoloPy/0.1.104/
...
def hubble_distance_z(z, **cosmo):
    ...
    return  c_light_Mpc_s / (H_0 * e_z(z, **cosmo))
...
def age(z, use_flat=True, **cosmo):
    ...
    >>> cosmo = cd.set_omega_k_0(cosmo)
    >>> t = cd.age(6.0, **cosmo)
    >>> print("age at z=6.0 = %.3g Gyr" % (t/ Gyr_s))
...
def age_flat(z, **cosmo):
    ...
    >>> print("age at z=6.0 is %.3g Gyr" % (t/ Gyr_s))
    ...
    t_z = (2. * 
           ...
           ( H100_s * cosmo['h'] * 3. * numpy.sqrt(lam)))
...
def redshift_d_light(dl, z_guess = 6.0, fmin_args={}, **cosmo):
    ...
    >>> cosmo = {'omega_M_0' : 0.3, 'omega_lambda_0' : 0.7, 'h' : 0.72}
    >>> cosmo = cd.set_omega_k_0(cosmo)
    >>> z = cd.redshift_d_light(10. *  c_light_Mpc_Gyr, **cosmo)
